...
Ambiguous submission recovery and retry are separate operations. A `Submitting` Attempt with an uncertain scheduler reply must reuse its original Attempt number and durable submission receipt; it must never allocate N+1. A new retry is only legal after a terminal Attempt and is daemon-owned. The `retry_run_v1` contract carries `run_id`, `expected_attempt_no`, a durable client `request_id`, and bounded resource overrides. Allocation happens under an immediate SQLite transaction so one request maps to exactly one N+1 Attempt and stale/concurrent requests fail closed rather than spawning duplicate scientific execution. Submission acceptance is also an immediate-transaction compare-and-set: the first writer to attach a scheduler JobID or Local handle records the status/event, a same-handle follower is idempotent and returns the newer canonical Run without rewriting it, and a different handle is a hard conflict. This prevents foreground Retry and daemon recovery from duplicating timeline events or regressing a newer Running/terminal snapshot while they race to adopt the same durable receipt.

Human Retry is fail-closed on **both** durable continuation binding state and residual Run identity. A Run counts as human/unbound only when continuation status is known, no `ContinuationBinding` exists, and `RunRecord.session_id`, `agent`, and `project_root` are all absent. If those identity fields survive without a binding row (for example after adoption or inconsistent historical state), runwatchd rejects `retry_run_v1` and the GUI does not expose Retry. The GUI must never infer permission from a missing binding row when the canonical Run still carries agent identity; repair/retry of that workflow belongs to the owning Agent Integration.
