...
- [ ] Resolve this acceptance boundary against production semantics before starting a new formal authority. pi-runs production `buildSubmitSpec()` emits `resources:{}` for Local Process and discards scheduler-only raw resource fields before `submit_run_v2`; any verifier adjustment must be coupled to that production normalization and must remain strict for durable identity/command/workspace and Slurm/LSF resources.
