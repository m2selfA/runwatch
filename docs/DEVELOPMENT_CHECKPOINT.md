...
### R11d — endurance/fault hardening — completed 2026-09-03
...
- [x] pi-runs commit **`bcc9eaf` (`test: decouple durable token proof from provider echo`)** hardens that acceptance boundary while leaving runwatch durability semantics unchanged. Exact token verification remains authoritative in a successful `read`/`ssh_read` tool result; completion/settlement exactly-once, verification-tool allowlist/count/order, no resubmit/`runs_wait`, and one final `stop` acknowledgement bound to the completed Run remain hard gates. The provider's actual acknowledgement is preserved in evidence rather than rewritten. Focused release tests pass **6/6** and default pi-runs tests remain **53 passed / 0 failed / 1 skipped** with the real Pi loader passing.
- [x] The hardened verifier was replayed read-only against authority #3's actual round-2 Pi JSONL and validates one completion, one settlement, `runs_status/runs_logs/read`, plus the exact token tool result while explicitly recording `success_marker_exact=false` for the provider's imperfect redundant echo. No runwatch DB, evidence file or Pi JSONL was modified.
- [x] pi-runs formal authority #4 started at `acceptance-output/soak-20260903011346-0e69c771` from `bcc9eaf` with the same packaged runwatch/fault contract and a bounded 1800 s segment against target 7200 s. Round 1 is clean: Local `local:91176:01dd3b417f4560e1` + gm00 Slurm Job **31772** survived serve **91196 -> 89372** and each finished Delivery=1, AgentInvocation=1, completion=1, settlement=1 with the exact Local/Slurm verification sequences.
- [x] Authority #4 round 2 is clean. Local `local:43320:01dd3b4325c65e40` reached real same-session `needs_rebind` with zero wrong-branch completion/settlement, then one `runs_rebind` recovered the same Delivery on attempt 2 and final completion/settlement=1/1. gm00 Slurm Job **31773** remained the same running JobID through isolated SSH `fresh -> unreachable(source=transport, Channel send error) -> fresh(source=scheduler)` and then completed exactly once; serve recovered **89372 -> 22504**.
- [x] Authority #4 round 3 / segment 1 closed cleanly. Local `local:93524:01dd3b44bb9967f7` and gm00 Slurm Job **31775** survived serve **22504 -> 58068**. Local then reached completion=1/settlement=0 before the scheduled crash; isolated serve **58068 -> 95088** recovered the same Delivery with attempts=2 / Invocations=2 and final completion/settlement=1/1. Slurm also survived the global crash with attempts=2 / invocations=2 and one final settlement. Segment 1 exited 0 after **2229.353 s / 3 rounds / 6 cases**; read-only reporting has no dirty segments and coverage restart=3, SSH=1, rebind=1, settlement-crash=1.
- [x] pi-runs fixed the acceptance-only cleanup defect without changing runwatch or serializing combined faults. Concurrent inspectors now pass through an all-settled barrier: every inspector starts concurrently, all siblings settle before the first rejection is propagated, and `runRound()` therefore reaches its child cleanup without orphaning another inspector's timers/polls. Focused soak regression passes **17/17**; default pi-runs regression is **55 passed / 0 failed / 1 skipped** with the real Pi loader passing. Authority #4 remains preserved as clean historical evidence and is intentionally not resumed because the acceptance tree changed.
- [x] Final pi-runs authority #5 started at `acceptance-output/soak-20260903022319-d0074e45` from cleanup-fixed `596d2f6`, retaining packaged runwatch `rc-4888d39`, the 7200 s target and the same mixed Local+gm00 fault cadence. Round 1 is clean: Local `local:90324:01dd3b4b3acaedbe` + Slurm Job **31781** were active across serve **93556 -> 55368** and both settled exactly once with the expected Local/Slurm verification sequences.
- [x] Authority #5 round 2 is clean. Local `local:94864:01dd3b4ce53d4b1b` produced real same-session divergence with zero wrong-branch completion/settlement; one explicit `runs_rebind` recovered the same Delivery on attempt 2 and final completion/settlement=1/1. gm00 Slurm Job **31782** remained the same `running` JobID through isolated SSH `fresh -> unreachable(source=transport, Channel send error) -> fresh(source=scheduler)`, then settled exactly once with the expected remote verification sequence. Serve recovered **55368 -> 49176**.
- [x] Authority #5 round 3 / segment 1 closed cleanly. Local `local:89256:01dd3b4e80e5ff1b` + gm00 Slurm Job **31784** survived normal serve **49176 -> 89912**. Local then hit completion=1/settlement=0 before isolated serve **89912 -> 94808**; recovery completed the same Delivery with attempts=2 / Invocations=2 and final completion/settlement=1/1. Slurm also survived that global crash with attempts=2 / Invocations=2 and one final settlement. Segment 1 exited 0 after **2174.382 s / 3 rounds / 6 cases**; read-only evidence has `dirty_segments=[]`, restart=3, SSH=1, rebind=1 and settlement-crash=1.
- [x] Authority #5 round 4 is clean and establishes the second real SSH + rebind recoveries. Local `local:90252:01dd3b507ece6c4d` reached same-session divergence with zero wrong-branch completion/settlement; exactly one explicit `runs_rebind` recovered the same Delivery on attempt 2 and final completion/settlement=1/1. gm00 Job **31787** remained the same `running` JobID through `fresh -> unreachable(source=transport, Channel send error) -> fresh(source=scheduler)`, then settled exactly once with the expected remote verification sequence. Segment-2 serve recovered **96984 -> 96568**. Repeated SSH and rebind requirements are now satisfied.
- [x] Authority #5 is now intentionally **dirty/non-resumable** after segment 2 failed closed in round 5. The real Local initiating provider recovered from a 503, then supplied scheduler-only `mem="1G"` and `time="00:10:00"` on `runner=process`; the frozen pi-runs verifier rejected the first non-neutral extra (`mem`) and preserved `segment-0002/failure.json` after **760.528 s** / one completed round. Read-only reporting returns `dirty_segments=[failed:2]`, so none of segment 2 is credited or resumable. Segment-2 supervisor/serve PIDs 96984/96568 are confirmed stopped by `finally`.
- [x] pi-runs closed the Process optional-resource false-negative by using production `buildSubmitSpec()` itself in the acceptance verifier. Local Process scheduler-only extras are accepted only when they normalize to the identical `resources:{}` runwatch spec; the same non-neutral resource on Slurm/LSF changes the normalized spec and still fails. Unknown args and non-neutral retired wakeup/webhook values remain hard failures; durable run_id/name/runner/command/workspace remain strict. pi-runs default regression is **55 passed / 0 failed / 1 skipped**, and authority #5's real round-5 seed replays successfully read-only under this rule without changing old evidence.
- [x] Formal pi-runs authority #6 started at `acceptance-output/soak-20260903032005-11ed23b0` from corrected `76cf02f`, with frozen contract-tree SHA-256 `d06a3b5c962da2fa0e8d222e60ec41f1e0f558f5f27c9a81ed623e7d8b90a43a` and the same packaged runwatch/7200 s mixed-fault contract. Round 1 is clean: Local `local:53348:01dd3b5330c613a0` + gm00 Slurm Job **31789** were active across serve **97376 -> 98184** and both settled exactly once with the expected Local/Slurm result-inspection sequences. The Slurm seed recovered automatically from one provider 503 inside the bounded seed timeout. A burst of named-pipe `os error 232` log messages occurred while both Runs stayed live; final round evidence remained fully correct, so retain it as an audit observation rather than overriding the formal invariants.
- [x] Authority #6 segment 1 closed cleanly after **2249.112 s / 3 rounds / 6 cases**. Round 2 established the first scheduled SSH + rebind recoveries: Local `local:8976:01dd3b54c918427d` had zero wrong-branch completion/settlement before exactly one `runs_rebind`, then final completion/settlement=1/1 on Delivery attempt 2; gm00 Job **31790** stayed the same running JobID through `fresh -> unreachable(source=transport, Channel send error) -> fresh(source=scheduler)` and settled exactly once. Round 3 then established the first completion-before-settlement crash recovery: Local `local:89760:01dd3b566af86fb7` reached completion=1/settlement=0 before isolated serve **97308 -> 93864**, recovered with attempts=2 / Invocations=2 and one final settlement; Job **31791** likewise survived the global crash. Read-only summary is `dirty_segments=[]`, active elapsed **2249.112 s**, restart=3, SSH=1, rebind=1, settlement-crash=1.
- [x] Authority #6 round 4 passed before the later segment failure. Local `local:97864:01dd3b5f5e9463c2` completed the second real same-session rebind recovery with zero wrong-branch completion/settlement, exactly one `runs_rebind`, exact result inspection and one final settlement. gm00 Job **31792** passed the paired scheduled SSH-loss/recovery verifier and settled exactly once; segment-2 serve recovered **94912 -> 71384**.
- [x] Authority #6 is now **dirty/non-resumable** after round 5 materially changed the Local workspace/command. The real provider rewrote `acceptance-output\\soak-20260903032005-11ed23b0\\...` to `acceptance-output\\soak\\20260903032005-11ed23b0\\...`; production `runs_submit` rejected the nonexistent Local workspace before launch and the strict verifier independently rejected the semantic mismatch. Slurm sibling Job **31793** had already armed, was confirmed RUNNING and was explicitly cancelled; segment-2 supervisor/serve PIDs 94912/71384 are stopped. `failure.json` records **702.391 s / one completed round** and read-only reporting credits only segment 1 (**2249.112 s**) with `dirty_segments=[failed:2]`.
- [x] pi-runs removed the long-Windows-path transcription hazard without weakening normalized workspace/command equality. Formal Local soak cases now use stable `acceptance-output\\local-workspace` and a unique relative marker command; exact result inspection still resolves the absolute marker file and production `buildSubmitSpec()` comparison remains strict. Focused tests pass **6/6**, default regression remains **55 passed / 0 failed / 1 skipped**, and real-provider Local smoke `soak-20260903050529-fbeb6359` passed **143.891 s / exit 0** with the actual Pi tool call using the short workspace and relative marker.
- [x] Formal pi-runs authority #7 started at `acceptance-output/soak-20260903050918-ff7f558d` from `ab13e7d`, frozen contract-tree SHA-256 `3d1f46dfe8c68a891ee48852b51f42ebf594f6cb70e2f864a52c46aa45113227`. Round 1 passed the full 600-second mixed workload: Local `local:54464:01dd3b626abc6639` + gm00 Job **31794** both transitioned `Running -> Succeeded`, the full exactly-once/result-inspection verifier returned, and round 2 was created. Scheduled serve restart was **95776 -> 100336**.
- [x] Authority #7 round 2 passed the first scheduled SSH + branch-rebind pair. Local `local:97124:01dd3b640a740ee4` recovered through the real same-session rebind gate; gm00 Job **31795** passed the isolated same-JobID SSH-loss/recovery verifier and final exactly-once inspection. The round advanced to round 3; scheduled serve restart was **100336 -> 90924**.
- [x] Authority #7 segment 1 closed cleanly at **2176.025 s / 3 rounds / 6 cases**. Round 3 established the first completion-before-settlement recovery: Local `local:96360:01dd3b65ae9c7d89` + Job **31796** were active across serve **90924 -> 96384**; Local then persisted completion=1/settlement=0 before isolated serve **96384 -> 95748**, recovered with Delivery attempts=2 / Invocations=2 and final completion/settlement=1/1. Job 31796 likewise survived the global crash with one final settlement. Coverage is restart=3, SSH=1, rebind=1, settlement-crash=1, dirty=0.
- [x] Authority #7 segment 2 closed cleanly at **2277.415 s / rounds 4-6 / 6 cases**, cumulative **4453.440 s / 6 rounds / 12 cases** with no dirty segments. Round 4 completed the second SSH + branch-rebind pair on Local `local:98596:01dd3b67cc944a66` + gm00 Job **31797**; round 5 Local `local:97656:01dd3b696b8467e9` + Job **31798** passed normally; round 6 combined SSH + rebind + completion-before-settlement crash on Local `local:101068:01dd3b6b1ab2af0c` + Job **31799** and still finished with one final completion/settlement per case. Machine coverage is restart=6, SSH=3, rebind=3, settlement-crash=2, so every repeated-fault requirement is satisfied.
- [x] Authority #7 round 7 passed Local `local:102224:01dd3b718cd7b3da` + gm00 Job **31800** across serve **101076 -> 100428**. Slurm seed recovered from a real provider 503; its exact-session offline continuation later completed `runs_status/runs_logs/ssh_activate/ssh_read`, read the exact token, and absorbed two provider stall-watchdog retries before a single terminal acknowledgement/settlement. Final Local and Slurm Delivery attempts remain 1, AgentInvocation count 1, completion=1 and settlement=1; round 8 was created only after this verifier passed.
- [x] Authority #7 round 8 passed Local `local:100024:01dd3b7492d8d0c6` + gm00 Job **31801** across serve **100428 -> 96588**. Job 31801 remained the same running JobID through `fresh -> unreachable(source=transport, Channel send error) -> fresh(source=scheduler)`. Local sibling divergence had completion=0/settlement=0 before exactly one `runs_rebind`; the same Delivery finished on attempt 2 with Invocation=1 and final completion/settlement=1/1 plus exact `runs_status/runs_logs/read`. Slurm finished attempt=1/Invocation=1/completion=1/settlement=1 with `runs_status/runs_logs/ssh_activate/ssh_read`. Round 9 was created only after both inspectors passed.
- [x] Formal pi-runs authority #7 is **v1 endurance qualified**. Segment 3 closed cleanly at **3058.683 s / rounds 7-9 / 6 cases**, yielding **7512.123 s clean active / 3 segments / 9 rounds / 18 cases / 0 failed segments**. Round 9 Local `local:97096:01dd3b7680940149` + gm00 Job **31802** both succeeded; the Local session had exactly completion=1/settlement=0 before isolated serve **100024 -> 87468**, then orphan recovery used a second Invocation and reached one final settlement. Slurm recovered from the same crash with attempts=2 / Invocations=2 and one completion/settlement. Machine qualification is true with Local=9, Slurm=9, serve restart=9, SSH recovery=4, rebind recovery=4, settlement-crash recovery=3 and `reasons=[]`.
- [x] Final RC replay exposed a fast-terminal live/offline ownership race that authority #7's 600-second workloads did not exercise. Real Local release evidence `local-process-20260903075214-a8369534` passed exactly once, but short gm00 Slurm Job **31804** completed while its initiating Pi was still live: the live client claimed Delivery `r8b_slurm_20260903075858-56a43330:a1:terminal`, then Pi exited before acknowledgement. The persisted Delivery remained `state=delivering, attempts=1` after both the live lease and 60-second claim expired, with `AgentInvocation=0`; no `runwatch/completion` reached the saved Pi session. The earlier Job **31803** failure is separate and was caused by provider quota exhaustion after `ssh_activate`.
- [x] Closed that durability gap in the canonical RunStore rather than in an adapter retry loop. Expired `delivering` claims are now requeued by the same transactional helper from both live `claim_deliveries()` and offline `reserve_offline_invocation()`, so a dead live owner cannot hide a terminal Delivery from the offline worker. Regression `expired_live_delivery_claim_is_recovered_by_offline_reservation` reproduces the Job-31804 shape and proves attempts advance 1 -> 2 with exactly one offline AgentInvocation. Post-fix `cargo check --all-targets` passes and `cargo test --all-targets` passes **106 / 0 failed / 8 ignored** (existing russh future-incompat warning only). Because this changes the release runtime after authority #7, preserve #7 as valid historical evidence but require a new current-binary formal endurance authority before tagging.
- [x] Packaged the fix from `f6b75b8` with Rust `xtask`: `dist/rc-f6b75b8/runwatch-v0.1.0-windows-x86_64.zip`, **9,686,763 bytes**, SHA-256 `3b67fae3055bed5765ae587fd9b27d228ea84fb47f1a0c4c377a0317591e426f`; `xtask verify` reports `ok=true`, platform `windows-x86_64`, 5 files. The package was extracted to a new isolated sibling layout without overwriting earlier candidates.
- [x] The fixed packaged short gm00 Slurm replay passed with real `localCPA/gpt-5.6-luna`: Job **31805**, Run `succeeded`, initiating `runs_doctor=1`/`runs_submit=1`, Delivery `delivered` attempt=1, exactly one completed AgentInvocation, session completion=1/settlement=1, and exact `runs_status/runs_logs/ssh_activate/ssh_read` result inspection. This particular timing exited Pi before terminal claim and therefore followed the normal offline path; the stale-live-claim takeover itself remains deterministically covered by the Job-31804 regression above.
- [x] A fixed-package Local replay naturally exercised the repaired stale-live-claim path: live claim attempt 1 expired, the same Delivery was recovered offline on attempt 2, exactly one AgentInvocation completed, and the exact Pi session still contained one completion, one settlement and exact `runs_status/runs_logs/read` token inspection. pi-runs therefore tightened its clean release verifier to allow only direct attempt=1 or one handoff retry attempt=2 when AgentInvocation count remains exactly 1; attempt>2/repeated Invocation/non-delivered shapes still fail. Focused release tests are **7/7**, full pi-runs regression is **56 passed / 0 failed / 1 skipped**, explicit Pi loader exits 0 and explicit live bridge passes **1/1**.
- [x] Committed-tree fixed-package Local replay `local-process-20260903084050-27b86cd8` passes direct attempt=1 with real `localCPA/gpt-5.6-luna`, one AgentInvocation, one completion/settlement and exact local result inspection.
- [x] Fresh formal authority #8 started at pi-runs `acceptance-output/soak-20260903084318-fa9d26c5` from pi-runs `4d27e15` + packaged runwatch `f6b75b8`. Frozen contract remains target 7200 s, Local+Slurm on gm00 `/share/home/shark/tmp`, delay 600 / seed timeout 480, restart=1, rebind=2, settlement-crash=3, SSH-fault=2 for 8 s; packaged executable SHA-256 is `3d1be0ac320947adbd29e5335a7894930e9ba8eb05366ae307ba39ccc0ac16a1` and pi-runs contract-tree SHA-256 is `6f9fcdf8d2fb2ff6f6f116e264ccfe666f3ef6df53052da97d027a3fee080a68`.
- [x] Authority #8 round 1 passed Local `local:39288:01dd3b804bc8d542` + gm00 Job **31806** across serve restart **100908 -> 97884**. Both final Runs succeeded with Delivery attempt=1, one completed AgentInvocation, completion=1/settlement=1 and the exact Local/Slurm result-inspection sequences. Round 2 started automatically on Local `local:100964:01dd3b81dac5f97d` + Job **31807**, carrying the first scheduled SSH-loss + branch-rebind pair.
- [x] Authority #8 round 2 passed the first SSH + branch-rebind pair on the fixed binary: serve **97884 -> 9680**, gm00 Job **31807** remained the same running JobID through `fresh -> unreachable(transport) -> fresh(scheduler)`, and Local wrong-branch completion/settlement remained 0/0 until one explicit rebind recovered the same Delivery on attempt 2. Both final sessions settled exactly once.
- [x] Authority #8 is preserved **failed/non-resumable** after segment 1 ended at **2100.404 s / 2 completed rounds**. Round 3 product evidence was healthy: gm00 Job **31808** succeeded and the exact Pi session completed `runs_status/runs_logs/ssh_activate/ssh_read`, read the exact token and persisted one final settlement. The failure was only a provider typo in the synthetic final acknowledgement, so the fail-closed verifier correctly rejected the segment and no #8 time is creditable.
- [x] pi-runs acceptance scaffolding now removes that transcription hazard without relaxing runtime semantics: the final acknowledgement is the exact fixed token `R8B_RELEASE_OK`; marker filenames are deterministic fixed-length `r8b-<10hex>.txt`; run_id, tool ordering, exact marker path and exact marker token remain independently strict. Focused tests pass 7/7, full pi-runs regression 56/0/1, and real fixed-package Local `r8b_local_20260903093219-cedc027f` plus gm00 Job **31810** both pass the revised gate.
- [x] Revised pi-runs acceptance tree committed as `9048141`; fresh authority #9 is frozen at `acceptance-output/soak-20260903093547-cade1766` against unchanged packaged runwatch `f6b75b8`. Contract remains target 7200 s, mixed Local+Slurm on gm00 `/share/home/shark/tmp`, delay 600 / seed timeout 480, restart=1, rebind=2, settlement-crash=3, SSH-fault=2 for 8 s. Frozen runwatch executable SHA-256 is `3d1be0ac320947adbd29e5335a7894930e9ba8eb05366ae307ba39ccc0ac16a1`; pi-runs contract-tree SHA-256 is `e3564ca39dce09f4a00631eb5ef9bb17f1beb7b0a8a8e354447d91fdb28732c6`. Never resume #8; only #9 may accumulate current-binary endurance time.
- [x] Authority #9 segment 1 passed **678.723 s / round 1 / 2 cases** with no failed segments. Local `local:18352:01dd3b87a02174c3` and gm00 Job **31811** remained live across serve **101620 -> 5880**; both reached `Succeeded`, exact short marker/token verification succeeded, exact `R8B_RELEASE_OK` was persisted, and each session has one final `runwatch/completion-settled`. Coverage is Local=1, Slurm=1, restart=1 and zero dirty evidence.
- [x] Authority #9 segment 2 is preserved **failed/non-resumable** at **679.592 s / round_start=2 / 0 credited rounds**. Local `local:4344:01dd3b897d5b3744` successfully rebound once (`rebound=true/reset_deliveries=1`), then completed exact status/log/read/token verification and one settlement. Slurm Job **31812** stayed the same running JobID through `fresh -> unreachable(Channel send error) -> unreachable(os error 10053) -> fresh`, then succeeded with exact status/logs/`ssh_activate`/`ssh_read`, exact fixed acknowledgement and one settlement. The only failure was the rebind-specific acceptance prompt still requesting the retired long success acknowledgement; therefore #9 cannot be resumed and its clean segment 1 is not release-creditable.
- [x] pi-runs unified the success acknowledgement structurally in committed tree `7d7755e`: one exported `RELEASE_SUCCESS_ACK`, one exported `buildExplicitRebindPrompt()`, zero long-form `R8B_RELEASE_OK:` success strings under `scripts/acceptance/`. Focused tests pass **25/25** and full regression **57 / 0 failed / 1 skipped**.
- [x] Real focused Local rebind qualification passed on unchanged packaged runwatch `f6b75b8`: `soak-20260903100356-5daaa502`, **114.964 s / 1 case / rebind_recoveries=1 / 0 failed segments**.
- [x] Fresh authority #10 is frozen from pi-runs `7d7755e` at `acceptance-output/soak-20260903100750-6e025696` against unchanged packaged runwatch `f6b75b8`. Frozen executable SHA-256 remains `3d1be0ac320947adbd29e5335a7894930e9ba8eb05366ae307ba39ccc0ac16a1`; pi-runs contract-tree SHA-256 is `965ee2ed857f7e975767932b205aa5e9655503d0eecd6117b2084e72138f3557`.
- [x] Authority #10 segment 1 passed **681.265 s / round 1 / 2 cases** with zero dirty evidence. Local `local:102404:01dd3b8c1aebe461` + gm00 Job **31813** stayed active across serve **103804 -> 87968**; coverage is Local=1, Slurm=1, restart=1.
- [x] Authority #10 segment 2 passed **687.335 s / round 2 / 2 cases**, cumulative **1368.600 s / 2 rounds / 4 cases / 0 failed segments**. Local `local:101792:01dd3b8db903bdbc` completed formal rebind under the unified prompt; gm00 Job **31814** stayed the same running JobID through `fresh -> unreachable(Channel send error) -> unreachable(os error 10054) -> fresh`; serve restarted **102816 -> 103536**. Coverage is restart=2, SSH=1, rebind=1 and no dirty evidence.
- [x] Authority #10 segment 3 passed **2181.907 s / rounds 3–5 / 6 cases**, cumulative **3550.507 s / 5 rounds / 10 cases / 0 failed segments**. Round 3 completed the first formal completion-before-settlement crash recovery: Local and Slurm terminal Deliveries each recovered from an interrupted first offline invocation to `delivered` with `attempts=2` and one final settlement. Round 4 completed the second real SSH-loss + branch-rebind pair on gm00 Job **31816** and Local `local:33620:01dd3b9148991df5`; coverage is restart=5, SSH=2, rebind=2, settlement-crash=1.
- [x] Authority #10 segment 4 is preserved **failed/non-resumable** at **762.062 s / round_start=6 / 1 completed round**. Round 6 itself passed the second formal completion-before-settlement crash recovery: Local `local:100416:01dd3b95e4ba804c` + gm00 Job **31818** reached terminal, serve was intentionally restarted **97584 -> 5556**, both persisted sessions later emitted exact `R8B_RELEASE_OK` and one final settlement, and the driver advanced to round 7. Round 7 then failed before submission because both real Pi seeds called `runs_doctor` once and received `ready=false` from `runwatch IPC timeout after 750 ms`; neither submitted a Run. The dirty segment cannot contribute round 6 or elapsed time, so #10 formal credit remains **3550.507 s / 5 rounds / 10 cases** through segment 3 and #10 must never be resumed.
- [x] pi-runs now waits for packaged runwatch IPC readiness at the start of every soak round before spawning any Pi seed. The Pi contract remains strict: each real Pi still calls `runs_doctor` exactly once and may submit only when that call returns `ready=true`. Focused soak tests pass **18/18**, full pi-runs regression remains **57 / 0 failed / 1 skipped**, and real focused crash-to-next-round evidence `soak-20260903113349-76a198fb` passes **363.517 s / 2 Local rounds / settlement_crash_recoveries=2 / 0 failed segments** on unchanged packaged runwatch `f6b75b8`.
- [x] pi-runs readiness-barrier tree committed as `7bfb40b`; fresh authority #11 is frozen at `acceptance-output/soak-20260903114134-5b2db744` against unchanged packaged runwatch `f6b75b8`. Frozen runwatch executable SHA-256 remains `3d1be0ac320947adbd29e5335a7894930e9ba8eb05366ae307ba39ccc0ac16a1`; new pi-runs contract-tree SHA-256 is `14f1a05f6db7718409d77ff73e2f33a01d39cd0084cbaf6e5b29eb088db2494b`.
- [x] Authority #11 segment 1 closed cleanly at **2775.753 s / 4 rounds / 8 cases / 0 failed segments**. Coverage is Local=4, Slurm=4, restart=4, SSH recovery=2, rebind recovery=2 and settlement-crash recovery=1. Round 4 completed the second SSH-loss + rebind pair on gm00 Job **31822** plus Local `local:102452:01dd3b9e1515df24`; the same JobID recovered from transport loss and the Local Delivery rebound exactly once before final settlement. Machine qualification is now blocked only by active elapsed below 7200 s and settlement-crash recovery not yet repeated.
- [x] Authority #11 segment 2 closed cleanly at **2800.818 s / rounds 5–8 / 8 cases**, cumulative **5576.571 s / 8 rounds / 16 cases / 0 failed segments**. Coverage is restart=8, SSH recovery=4, branch rebind recovery=4 and settlement-crash recovery=2. Every non-duration formal endurance gate is now satisfied; machine `qualified=false` has only `active_time_meets_target=false`, with **1623.429 s** remaining.
- [x] Authority #11 segment 3 closed cleanly at **2223.649 s / rounds 9–11 / 6 cases**, giving the formal current-binary authority **7800.220 s / 11 rounds / 22 real cases / 3 clean segments / 0 failed segments**. Machine `v1_endurance.qualified=true`; coverage is Local=11, Slurm=11, serve restart=11, SSH recovery=5, branch rebind recovery=5 and settlement-crash recovery=3, with every formal requirement true and `reasons=[]`. Preserve `acceptance-output/soak-20260903114134-5b2db744` as the release authority; no further endurance resume is required.
- [x] Final RC replay passed on 2026-09-03 without changing the qualified runtime: runwatch `cargo fmt -- --check`, `cargo check --all-targets` and `cargo test --all-targets` are green (**106 passed / 0 failed / 8 ignored**, existing `russh 0.54.5` future-incompat warning only); pi-runs `npm test` is **57 / 0 failed / 1 skipped**; explicit Pi loader exits 0; explicit real-Pi live bridge passes **1/1**; fixed-package Local evidence `local-process-20260903143640-87bf00a7` passes packaged doctor/submit + exactly-once completion/settlement; fixed-package gm00 Slurm evidence `slurm-20260903143800-2e98a872` / Job **31830** passes packaged doctor/submit + exact `runs_status/runs_logs/ssh_activate/ssh_read`; `xtask verify` remains `ok=true` for ZIP SHA-256 `3b67fae3055bed5765ae587fd9b27d228ea84fb47f1a0c4c377a0317591e426f`; final read-only authority report remains `qualified=true`, **7800.220 s**, `dirty_segments=[]`. Authority #7 remains historical qualification; #8/#9/#10 remain immutable failed evidence.
- [x] Endurance failures remained fail-closed throughout qualification: dirty authorities were preserved and never washed out, no adapter-specific retry loop replaced canonical Delivery ownership, and only clean authority #11 is accepted as current-binary release evidence.

### R11e — compatibility retirement + release candidate contract — completed 2026-09-03
...
- [x] `docs/V1_RELEASE_CANDIDATE.md` is the canonical v1 scope/compatibility/package/gate checklist. R11d endurance and the final RC replay are now complete; the candidate is ready for coordinated `v0.1.0` tag/release-note preparation, with no new AgentAdapter work in scope.
...
- [x] Real 1080×720 acceptance found a fixed-height layout bug where the 280px Runs table could squeeze Detail tab content to effectively zero height. The table now uses flex weight so dashboard and detail share the available height; Logs/Timeline controls remain visible at the default size.
- [x] Dashboard rows are now a hard single-line surface: WindUI `table_virtual(...).cell_lines(1)` maps to label `max_lines=1`, so Name/Runner/Host/Handle/Observation/Continuation/Updated clip rather than wrap. `Updated` also receives a larger width weight (1.2) so its normal elapsed text remains readable without shifting wrapping into Continuation.
...
- [x] Real Limited-session acceptance found two Windows Service defects and closed both. Removal now disables the Task registration before `/End`/bounded owner shutdown/`/Delete`, then best-effort re-enables it on failure; a real 70-second hold crossed the PT1M trigger interval without the removed supervisor resurrecting.
- [x] A Task installed from an elevated process previously left `CAP00\\inter` with read-only access, so the ordinary desktop GUI received `ERROR: Access is denied`. Installation now reads the current token SID and writes a Task `SecurityDescriptor` granting Generic All to current user + SYSTEM + Administrators. Elevated-install inspection showed explicit `CAP00\\inter FullControl`; a separate Limited Session 2 `autostart --remove` then returned exit 0 and removed the Task.
...
#### R13f — usability/release qualification — completed 2026-09-04
...
- [x] WindUI debug-only fixture mode renders Dashboard / Detail / Daemon-offline screenshots at **1080×720**. The final single-line Dashboard smoke is **1080×720 / 51,103 bytes / exit 0**; earlier Detail/Offline fixtures remain part of the same off-screen QA surface.
...
- [x] Rust-native Windows package builds successfully from an isolated target directory so a user-running GUI never needs to be killed. The post-interactive-fix local candidate `runwatch-v0.2.0-windows-x86_64.zip` is **10,314,458 bytes**, SHA-256 `6b83b127959b7374a8873469fb3d32240fd460002cbc08fc77276d39437378d5`; final `xtask verify` is `ok=true`, files=5, version `0.2.0`, platform `windows-x86_64`.
...
- [x] GitHub fresh-runner CI for implementation commit `12cc8d2bc81786ec908dfb89f76e31079c75213c` closed green in workflow run **33837473979**: Windows `windows-2022` completed NASM -> fmt/check -> **120 tests** -> `xtask package` -> `xtask verify` -> artifact upload; Linux completed the manylinux2014 `glibc 2.17` check/test/release path, final `readelf` gate and artifact upload. Both shipped Linux ELFs still require at most **GLIBC_2.16**, below the 2.17 ceiling.
- [x] The final Windows GitHub artifact is ID **9923952858**, `runwatch-windows-x86_64-12cc8d2bc81786ec908dfb89f76e31079c75213c`, **10,239,890 bytes**, artifact digest `sha256:bc75f64a0a8f08cd27c6f79007b280b1ab1a6bc7a8e130d84841672c9f48f0f2`. Download inspection confirms it wraps `runwatch-v0.2.0-windows-x86_64.zip`, **10,254,957 bytes**, SHA-256 `c9850661c006e078266f2c5d3d52d3d677656a23d174e2135e178c67859d55b4`; its manifest reports version `0.2.0`, platform `windows-x86_64`, sibling binaries required and five payload files.
- [x] The final Linux GitHub artifact is ID **9923837745**, `runwatch-linux-x86_64-glibc217-12cc8d2bc81786ec908dfb89f76e31079c75213c`, **6,127,168 bytes**, artifact digest `sha256:6d17638accb0fcb6183cadb99a6ea0735a2706576d616067f2793d1456ceab3e`.
- [x] Final packaged interactive Windows acceptance closed in the real **RDP Session 2** with the ordinary Limited `CAP00\\inter` desktop token. Selected Run drill-in copied the exact Run ID; `Logs 500 -> Copy` returned a real bounded tail (3,918 bytes with the disposable Run marker); `Probe now` advanced canonical `observed_at`; Cancel dialog Back was non-destructive, Confirm converged the disposable Local Process Run to `cancelled` and the process exited; `WM_CLOSE` left the GUI process alive while the window became hidden.
- [x] Service acceptance used a separate disposable Local Process Run: the Limited GUI removed the Task and daemon while the scientific process survived, remained offline for **70 seconds** across a full PT1M scheduler repetition window without resurrection, then the same GUI restored the Task/daemon with Action pointing to the current package. A separate offline -> enable acceptance also confirmed the GUI-created Task gives the current user `FullControl`.
...
- [x] Added equal-size Host cards. A single preferred width is derived from the longest alias/endpoint/route/usage value across the inventory (bounded to 280..390 logical px); the longest wrapped route determines the common card height. Shorter cards never collapse independently.
...
A writer-endurance smoke then exposed a narrower concurrent-acceptance race: the foreground `retry_run_v1` writer and daemon pending-retry recovery could both adopt the **same** remote receipt. Scientific exactly-once still held (one Attempt-2 JobID and one execution marker), but both stale writers appended `scheduler_submitted`, and the later queued snapshot could theoretically overwrite a more advanced canonical state. R17 now uses a SQLite Immediate-transaction submission-acceptance CAS: the first writer atomically records handle/status/event; same-handle followers are idempotent and return the canonical Run without another event; a different handle is a hard conflict. The deterministic unit gate also proves an old queued acceptance cannot regress a canonical Running Attempt. The real ambiguous-receipt gate was rerun after the fix: Attempt 1 Job **31853**, Attempt 2 Job **31854**, exactly two Attempts, exactly one scientific Attempt-2 execution, and exactly one `scheduler_submitted` event per Attempt.
...
A later fail-closed audit found one reachable inconsistent identity shape before formal endurance could be credited: `adopt_run_v1` can leave `RunRecord.session_id` / `agent` / `project_root` populated even when no `continuation_bindings` row exists. Treating only the binding table as the Human Retry gate could therefore misclassify that Run as unbound and let N+1 strip the residual agent identity. R17 now rejects this state at both boundaries: `create_retry_intent` refuses any canonical Run that still carries agent identity without a durable binding, and the GUI withholds `retry_context` and reports that Human Retry fails closed. Regression coverage constructs exactly this orphaned-identity shape with the desired Retry snapshot cleared, proving no Attempt 2 or pending retry intent is created; the GUI regression proves the same state cannot expose Human Retry. Post-fix `cargo check --all-targets` passes and `cargo test --all-targets` passes **133 / 0 failed / 10 ignored**.
