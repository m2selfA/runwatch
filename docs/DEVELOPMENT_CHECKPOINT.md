...
- [ ] Allow the already-frozen authority-#4 segment to close without changing runwatch or its acceptance tree; round 3 schedules completion-before-settlement crash coverage. A read-only pi-runs audit found a harness cleanup defect independent of runwatch durability: concurrent case inspection uses `Promise.all()`, so one rejection can propagate before a sibling inspector settles and leave its timer/poll handles alive. After this segment reaches a safe evidence boundary, pi-runs must preserve concurrent fault timing but await all inspectors before propagating failure, then start the final authority from the cleanup-fixed contract tree.
