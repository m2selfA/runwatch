...
- [ ] Start a **new frozen formal endurance authority** from the corrected `d279a58` verifier contract and continue until pi-runs `--report-evidence-dir` returns `v1_endurance.qualified=true`. Preserve both failed authorities (`b21f6ed7` and `4ddcb294`) as immutable evidence; do not resume, rewrite or count them toward the new authority.
