...
- [ ] Run a **new** formal multi-hour endurance evidence session from the current packaged release layout until pi-runs `--report-evidence-dir` returns `v1_endurance.qualified=true`. The failed `b21f6ed7` session remains immutable/non-resumable. The next frozen profile must use the new seed/workload timing invariant while repeatedly combining resident restart, SSH loss/recovery, scheduler completion, offline Pi relaunch, branch rebind and completion/settlement crash recovery.
