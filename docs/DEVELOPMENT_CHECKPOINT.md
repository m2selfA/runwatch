...
- [ ] pi-runs should remove the stochastic long-Windows-path transcription burden without weakening normalized workspace/command equality, using a short stable acceptance-only Local workspace plus relative unique marker. A new frozen authority is required afterward.
