...
- [ ] Run the final multi-hour concurrent endurance matrix from the current packaged release layout, combining resident serve restart, transient SSH loss/recovery, scheduler completion, offline Pi relaunch, live branch rebind and completion/settlement crash recovery across repeated rounds. The focused fault dimensions are individually green; duration/repetition under one resident authority remains the release blocker.
