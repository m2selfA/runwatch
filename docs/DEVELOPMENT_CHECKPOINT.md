...
- [ ] Continue authority #3 until pi-runs `--report-evidence-dir` returns `v1_endurance.qualified=true`. The current segment is still in progress, so no active-time credit is claimed yet. Preserve both failed authorities (`b21f6ed7` and `4ddcb294`) as immutable evidence; do not resume, rewrite or count them toward authority #3.
