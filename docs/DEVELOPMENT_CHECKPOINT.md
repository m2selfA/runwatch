...
- [ ] Before the final authority, pi-runs must fix the acceptance-only cleanup defect without changing runwatch semantics: concurrent `Promise.all()` inspection can propagate one rejection before its sibling inspector settles and leave timers/polls alive. Preserve concurrent fault timing but await all inspectors before propagating failure, regression-test it, then start a fresh authority because `scripts/acceptance` changes the frozen contract tree. Authority #4 remains preserved as clean historical evidence and is not resumed.
