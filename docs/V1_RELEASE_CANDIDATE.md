...
- authority #9 is now preserved failed/non-resumable after segment 2 ended at **679.592 s / round_start=2 / 0 credited rounds**. Product fault recovery itself passed: Local `local:4344:01dd3b897d5b3744` performed one successful rebind (`rebound=true/reset_deliveries=1`), exact status/log/read/token verification and one final settlement; gm00 Job **31812** stayed the same running JobID through `fresh(scheduler) -> unreachable(transport, Channel send error) -> unreachable(transport, os error 10053) -> fresh(scheduler)`, then succeeded with exact status/logs/`ssh_activate`/`ssh_read`, exact short acknowledgement and one settlement. The segment failed only because the rebind-specific acceptance prompt still requested the retired long success acknowledgement, so the short exact verifier correctly rejected it. #9 must never be resumed and its clean first segment is not release-creditable.
- pi-runs now exports one `RELEASE_SUCCESS_ACK` and builds rebind turns through one exported `buildExplicitRebindPrompt()` using that same constant; no `R8B_RELEASE_OK:` long-form success string remains under `scripts/acceptance/`. Focused release+soak tests pass **25/25**, full regression passes **57 / 0 failed / 1 skipped**, and real focused Local rebind `soak-20260903100356-5daaa502` passes **114.964 s / rebind_recoveries=1 / 0 failed segments** on unchanged packaged runwatch `f6b75b8`. Fresh authority #10 is required from committed pi-runs `7d7755e`; authorities #8/#9 remain immutable failed evidence.
- authority #10 is frozen from committed pi-runs `7d7755e` at `soak-20260903100750-6e025696` against unchanged packaged runwatch `f6b75b8`. Frozen executable SHA-256 remains `3d1be0ac320947adbd29e5335a7894930e9ba8eb05366ae307ba39ccc0ac16a1`; new pi-runs contract-tree SHA-256 is `965ee2ed857f7e975767932b205aa5e9655503d0eecd6117b2084e72138f3557`. Segment 1 passed cleanly at **681.265 s / round 1 / 2 cases** on Local `local:102404:01dd3b8c1aebe461` + gm00 Job **31813**, across serve **103804 -> 87968**, with zero dirty evidence.
- authority #10 segment 2 passed cleanly at **687.335 s / round 2 / 2 cases**, cumulative **1368.600 s / 2 rounds / 4 cases / 0 failed segments**. Local `local:101792:01dd3b8db903bdbc` completed formal branch rebind under the unified prompt; gm00 Job **31814** stayed the same running JobID through `fresh -> unreachable(Channel send error) -> unreachable(os error 10054) -> fresh`; serve restarted **102816 -> 103536**. Coverage is restart=2, SSH=1, rebind=1. Segment 3 is running with a larger per-segment budget to sweep rounds 3–5 while keeping the frozen authority contract unchanged.
