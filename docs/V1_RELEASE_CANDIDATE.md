...
- the original multi-hour endurance requirement was satisfied by authority #7, but the final replay legitimately invalidated current-binary qualification by discovering and fixing a runtime ownership race. Repackage the fixed runwatch, prove the short Slurm gate, then run a fresh frozen multi-hour authority before final closeout.
