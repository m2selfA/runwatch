...
- formal pi-runs authority #6 `soak-20260903032005-11ed23b0` is active from corrected `76cf02f`. Round 1 passed Local `local:53348:01dd3b5330c613a0` + Job **31789** across serve **97376 -> 98184**, with exactly-once Delivery/Invocation/completion/settlement and exact result inspection. One Slurm seed 503 recovered inside the bounded seed timeout; pipe-232 log noise did not alter the round's durable evidence.
- continue only authority #6 under its frozen 7200 s contract until `v1_endurance.qualified=true`; all dirty authorities remain preserved and uncredited.
