...
- production pi-runs maps Local Process to runwatch `resources:{}` and discards scheduler resource fields. The next verifier change must prove equivalence at that production-normalized boundary while remaining strict for identity/command/workspace and all remote scheduler resources. A new frozen authority is required afterward.
